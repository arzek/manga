<template lang="pug">

    section.base-section.uk-padding
        section.uk-comment.uk-margin-large-bottom
          header.uk-comment-header.uk-grid-medium.uk-flex-stretch(uk-grid)
            .uk-width-auto
              img.uk-comment-avatar(src="http://imgcover.mangachan.me/showfull_retina/uploads/posts/2012-07/thumbs/1341859425_i125583.jpg" width="340" height="340" alt="")
            .uk-width-expand
              h1.uk-comment-title
                a.uk-link-reset.uk-h1(href="#") Устроится на хорошую работу.
              ul.uk-comment-meta.uk-subnav.uk-subnav-divider
                li Манхва
                li 162 главы
                li +7799
                li 12 дней назад
              .uk-comment-body
                p
                  span.uk-badge.
                    школа
                  span.uk-badge.
                    комедия
                  span.uk-badge.
                    веб
                  span.uk-badge.
                    драма
                  span.uk-badge.
                    боевик
                  span.uk-badge.
                    боевые искусства
                  span.uk-badge.
                    гарем
                  span.uk-badge.
                    романтика
                  span.uk-badge.
                    школа
                  span.uk-badge.
                    школа
                p.
                  Данная манхва о молодом парне Сон Чхегу у которого умер отец, и ушла мать...
                  оставив его с братом и сестрой на произвол судьбы. Вскоре он поступает в школу где
                  намерен получить достойное образование, и в дальнейшем устроится на хорошую работу.
                  С этого момента и начинается его приключение в "Школе отвязных".
                p.
                  Данная манхва о молодом парне Сон Чхегу у которого умер отец, и ушла мать...
                  оставив его с братом и сестрой на произвол судьбы. Вскоре он поступает в школу где
                  намерен получить достойное образование, и в дальнейшем устроится на хорошую работу.
                  С этого момента и начинается его приключение в "Школе отвязных".
                p.
                  Данная манхва о молодом парне Сон Чхегу у которого умер отец, и ушла мать...
                  оставив его с братом и сестрой на произвол судьбы. Вскоре он поступает в школу где
                  намерен получить достойное образование, и в дальнейшем устроится на хорошую работу.
                  С этого момента и начинается его приключение в "Школе отвязных".
        div
          table.uk-table.uk-table-divider.uk-margin-remove-bottom
            thead
              tr
                th Название
                th.uk-table-shrink Загружено
            tbody
              tr
                td
                  a.uk-link-reset(href='#') Girl the Wild's   v2 - 260
                td.uk-table-shrink 2016-11-13
              tr
                td
                  a.uk-link-reset(href='#') Girl the Wild's   v2 - 260
                td.uk-table-shrink 2016-11-13
              tr
                td
                  a.uk-link-reset(href='#') Girl the Wild's   v2 - 260
                td.uk-table-shrink 2016-11-13
              tr
                td
                  a.uk-link-reset(href='#') Girl the Wild's   v2 - 260
                td.uk-table-shrink 2016-11-13
              tr
                td
                  a.uk-link-reset(href='#') Girl the Wild's   v2 - 260
                td.uk-table-shrink 2016-11-13
              tr
                td
                  a.uk-link-reset(href='#') Girl the Wild's   v2 - 260
                td.uk-table-shrink 2016-11-13
              tr
                td
                  a.uk-link-reset(href='#') Girl the Wild's   v2 - 260
                td.uk-table-shrink 2016-11-13
              tr
                td
                  a.uk-link-reset(href='#') Girl the Wild's   v2 - 260
                td.uk-table-shrink 2016-11-13
              tr
                td
                  a.uk-link-reset(href='#') Girl the Wild's   v2 - 260
                td.uk-table-shrink 2016-11-13
              tr
                td
                  a.uk-link-reset(href='#') Girl the Wild's   v2 - 260
                td.uk-table-shrink 2016-11-13
              tr
                td
                  a.uk-link-reset(href='#') Girl the Wild's   v2 - 260
                td.uk-table-shrink 2016-11-13
              tr
                td
                  a.uk-link-reset(href='#') Girl the Wild's   v2 - 260
                td.uk-table-shrink 2016-11-13
          ul.uk-pagination.uk-flex-center.uk-margin
            li
              a(href='#')
                span(uk-icon="icon: chevron-left")
            li
              a(href='#') 1
            li
              a(href='#') 2
            li.uk-active
              a(href='#') 3
            li.uk-disabled
              span ...
            li
              a(href='#') 40
            li
              a(href='#')
                span(uk-icon="icon: chevron-right")

</template>

<script>
  export default {
      name: 'manga-component'
  }
</script>